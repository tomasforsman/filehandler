...
namespace FileHandler.Components.Observers
{
    ...
    {    
            ...
            where T : class
        {
            ...
            Console.WriteLine("PostConsume: {0} TimeSpan: {1} ConsumerType: {2}", context.MessageId.Value.ToString(), duration.Milliseconds.ToString(), consumerType);
        ...
        public async Task ConsumeFault<T>(ConsumeContext<T> context, TimeSpan elapsed, string consumerType, Exception exception) where T : class
        {
            ...
            Console.WriteLine("ConsumeFault: {0} TimeSpan: {1} ConsumerType: {2} Exception: {3}", context.MessageId.Value.ToString(), elapsed.Milliseconds.ToString(), consumerType, exception);
